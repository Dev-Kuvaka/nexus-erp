...
frappe.provide("nexuserp.queries");
$.extend(nexuserp.queries, {
	user: function () {
	...
	item: function (filters) {
		var args = { query: "nexuserp.controllers.queries.item_query" };
		if (filters) args["filters"] = filters;
		return args;
	},

	bom: function () {
		return { query: "nexuserp.controllers.queries.bom" };
	},

	task: function () {
		return { query: "nexuserp.projects.utils.query_task" };
	},
	...
	employee: function () {
		return { query: "nexuserp.controllers.queries.employee_query" };
	},
	...
	get_filtered_dimensions: function (doc, child_fields, dimension, company) {
		...
		return {
			query: "nexuserp.controllers.queries.get_filtered_dimensions",
			filters: {
				dimension: dimension,
				account: account,
				company: company,
...
nexuserp.queries.setup_queries = function (frm, options, query_fn) {
	var me = this;
...
*/
nexuserp.queries.setup_warehouse_query = function (frm) {
	frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
		var row = locals[cdt][cdn];
		var filters = nexuserp.queries.warehouse(frm.doc);
		if (row.item_code) {
			$.extend(filters, { query: "nexuserp.controllers.queries.warehouse_query" });

frappe.provide("nexuserp.financial_statements");

nexuserp.financial_statements = {
	filters: get_filters(),
	baseData: null,
	formatter: function (value, row, column, data, default_formatter, filter) {
		const report_params = [value, row, column, data, default_formatter, filter];
		// Growth/Margin
		if (nexuserp.financial_statements._is_special_view(column, data))
			return nexuserp.financial_statements._format_special_view(...report_params);

		if (frappe.query_report.get_filter_value("report_template"))
			return nexuserp.financial_statements._format_custom_report(...report_params);

		if (frappe.query_report.get_filter_value("report_template"))
			return nexuserp.financial_statements._format_custom_report(...report_params);
		else return nexuserp.financial_statements._format_standard_report(...report_params);
	},
	...
	_format_custom_report: function (value, row, column, data, default_formatter, filter) {
		const columnInfo = nexuserp.financial_statements._parse_column_info(column.fieldname, data);
		const formatting = nexuserp.financial_statements._get_formatting_for_column(data, columnInfo);

		if (columnInfo.isAccount) {
			return nexuserp.financial_statements._format_custom_account_column(
				value,
				data,
				formatting,
				column,
			...
			return nexuserp.financial_statements._format_custom_value_column(
				value,
				data,
				formatting,
				column,
	...
	_format_custom_account_column: function (value, data, formatting, column, default_formatter, row) {
		...
		if (should_link_to_ledger) {
			...
			column.link_onclick =
				"nexuserp.financial_statements.open_general_ledger(" + JSON.stringify(glData) + ")";
		...
		// Style
		return nexuserp.financial_statements._style_custom_value(formattedValue, formatting, null);
	},
	...
	_format_custom_value_column: function (value, data, formatting, column, default_formatter, row) {
		...
		return nexuserp.financial_statements._style_custom_value(formattedValue, formatting, value);
	},
	...
	_format_standard_report: function (value, row, column, data, default_formatter, filter) {
		if (data && column.fieldname == nexuserp.financial_statements.name_field) {
			...
			if (data.account || data.accounts) {
				column.link_onclick =
					"nexuserp.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			}
			column.is_tree = true;
		}
	...
	onload: function (report) {
		...
		nexuserp.financial_statements.filters = get_filters();

		let fiscal_year = nexuserp.utils.get_fiscal_year(frappe.datetime.get_today());
...
function get_filters() {
	let filters = [
		...
		// accordingly.
		{
			...
			label: __("Currency"),
			fieldtype: "Select",
			options: nexuserp.get_presentation_currency_list(),
		},
		{
	...
	let fiscal_year = nexuserp.utils.get_fiscal_year(frappe.datetime.get_today(), false, false);
	if (fiscal_year) {

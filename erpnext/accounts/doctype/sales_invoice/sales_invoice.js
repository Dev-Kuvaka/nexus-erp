...
frappe.provide("nexuserp.accounts");

cur_frm.cscript.tax_table = "Sales Taxes and Charges";

nexuserp.accounts.taxes.setup_tax_validations("Sales Invoice");
nexuserp.accounts.payment_triggers.setup("Sales Invoice");
nexuserp.accounts.pos.setup("Sales Invoice");
nexuserp.accounts.taxes.setup_tax_filters("Sales Taxes and Charges");
nexuserp.sales_common.setup_selling_controller();
nexuserp.accounts.SalesInvoiceController = class SalesInvoiceController extends (
	nexuserp.selling.SellingController
) {
	setup(doc) {
	...
	onload() {
		...
		nexuserp.queries.setup_queries(this.frm, "Warehouse", function () {
			return nexuserp.queries.warehouse(me.frm.doc);
		});
		...
		nexuserp.queries.setup_warehouse_query(this.frm);
	}
	...
	refresh(doc, dt, dn) {
		...
		nexuserp.accounts.unreconcile_payment.add_unreconcile_btn(me.frm);
	...
	make_invoice_discounting() {
		frappe.model.open_mapped_doc({
			method: "nexuserp.accounts.doctype.sales_invoice.sales_invoice.create_invoice_discounting",
			frm: this.frm,
		});
	}
	...
	make_dunning() {
		frappe.model.open_mapped_doc({
			method: "nexuserp.accounts.doctype.sales_invoice.sales_invoice.create_dunning",
			frm: this.frm,
		});
	}
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "nexuserp.accounts.doctype.sales_invoice.sales_invoice.make_maintenance_schedule",
			frm: this.frm,
		});
	}
		...
		this.$sales_order_btn = this.frm.add_custom_button(
			...
			function () {
				nexuserp.utils.map_current_doc({
					method: "nexuserp.selling.doctype.sales_order.sales_order.make_sales_invoice",
	...
	quotation_btn() {
		...
		this.$quotation_btn = this.frm.add_custom_button(
			...
			function () {
				nexuserp.utils.map_current_doc({
					method: "nexuserp.selling.doctype.quotation.quotation.make_sales_invoice",
			...
			function () {
				nexuserp.utils.map_current_doc({
					...
					get_query: function () {
						...
						return {
							query: "nexuserp.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters,
						};
					},
					allow_child_item_selection: true,
	...
	customer() {
		...
		nexuserp.utils.get_party_details(
			this.frm,
			"nexuserp.accounts.party.get_party_details",
			{
		...
		if (this.frm.doc.customer) {
			frappe.call({
				method: "nexuserp.accounts.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				args: {
	...
	make_inter_company_invoice() {
		let me = this;
		frappe.model.open_mapped_doc({
			method: "nexuserp.accounts.doctype.sales_invoice.sales_invoice.make_inter_company_purchase_invoice",
			frm: me.frm,
		});
	}
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "nexuserp.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			frm: this.frm,
		});
	}
	...
	asset(frm, cdt, cdn) {
		...
		if (row.asset) {
			frappe.call({
				method: nexuserp.assets.doctype.asset.depreciation.get_disposal_account_and_cost_center,
				args: {
...
extend_cscript(cur_frm.cscript, new nexuserp.accounts.SalesInvoiceController({ frm: cur_frm }));

cur_frm.cscript["Make Delivery Note"] = function () {
	frappe.model.open_mapped_doc({
		method: "nexuserp.accounts.doctype.sales_invoice.sales_invoice.make_delivery_note",
		frm: cur_frm,
	});
};

cur_frm.cscript.income_account = function (doc, cdt, cdn) {
	nexuserp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
};

cur_frm.cscript.expense_account = function (doc, cdt, cdn) {
	nexuserp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
};

cur_frm.cscript.cost_center = function (doc, cdt, cdn) {
	nexuserp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "cost_center");
};
...
frappe.ui.form.on("Sales Invoice", {
	setup: function (frm) {
		...
		frm.set_query("income_account", "items", function (doc) {
			return {
				query: "nexuserp.controllers.queries.get_income_account",
				filters: {
					company: doc.company,
					disabled: 0,
				},
		...
		frm.set_query("time_sheet", "timesheets", function (doc, cdt, cdn) {
			return {
				query: "nexuserp.projects.doctype.timesheet.timesheet.get_timesheet",
		...
		frm.set_query("pos_profile", function (doc) {
			...
			return {
				query: "nexuserp.accounts.doctype.pos_profile.pos_profile.pos_profile_query",
				filters: {
					company: doc.company,
				},
			};
	...
	loyalty_points: function (frm) {
		if (frm.redemption_conversion_factor) {
			...
			frappe.call({
				method: "nexuserp.accounts.doctype.loyalty_program.loyalty_program.get_redeemption_factor",
				args: {
	...
	get_loyalty_details: function (frm) {
		if (frm.doc.customer && frm.doc.redeem_loyalty_points) {
			frappe.call({
				method: "nexuserp.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details",
				args: {
	...
	async get_timesheet_data(frm, kwargs) {
		return frappe
			.call({
				method: "nexuserp.projects.doctype.timesheet.timesheet.get_projectwise_timesheet_data",
				args: kwargs,
			})
			.then((r) => {
					...
					let d = new frappe.ui.Dialog({
						...
						fields: [
							...
							},
							{
								...
								get_query: () => {
									return {
										query: "nexuserp.controllers.queries.item_query",
										filters: {
											is_sales_item: 1,
											customer: frm.doc.customer,
											has_variants: 0,
...
var set_timesheet_detail_rate = function (cdt, cdn, currency, timelog) {
	frappe.call({
		method: "nexuserp.projects.doctype.timesheet.timesheet.get_timesheet_detail_rate",
		args: {
			timelog: timelog,
			currency: currency,
		},

...
import nexuserp
from nexuserp.accounts.deferred_revenue import get_deferred_booking_accounts
...
from nexuserp.setup.utils import get_exchange_rate as _get_exchange_rate
...
class JournalEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from nexuserp.accounts.doctype.journal_entry_account.journal_entry_account import JournalEntryAccount
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		from nexuserp.accounts.general_ledger import make_gl_entries
				...
				if not blank_row:
					blank_row = self.append(
						"accounts",
						{
							...
							"cost_center": nexuserp.get_default_cost_center(self.company),
						},
					)
...
class JournalEntryTaxWithholding:
	...
	def _calculate_net_total(self):
		from nexuserp.accounts.report.general_ledger.general_ledger import get_account_type_map
...
def get_outstanding(args):
	...
	company_currency = nexuserp.get_company_currency(args.get("company"))
	due_date = None
...
def get_account_details_and_party_type(account, date, company, debit=None, credit=None, exchange_rate=None):
	...
	company_currency = nexuserp.get_company_currency(company)
	account_details = frappe.get_cached_value(

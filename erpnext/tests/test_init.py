...
from nexuserp import encode_company_abbr
...
class TestInit(IntegrationTestCase):
	def test_encode_company_abbr(self):
		...
		names = [
			"Warehouse Name",
			"nexuserp Foundation India",
			f"Gold - Member - {abbr}",
			f" - {abbr}",
			"nexuserp - Foundation - India",
			f"nexuserp Foundation India - {abbr}",
			f"No-Space-{abbr}",
			"- Warehouse",
		]
	...
	def test_translation_files(self):
		...
		verify_translation_files("nexuserp")

	def test_patches(self):
		from frappe.tests.test_patches import check_patch_files

		check_patch_files("nexuserp")
